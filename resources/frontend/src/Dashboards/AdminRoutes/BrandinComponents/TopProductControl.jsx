...
const TopProductControl = () => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (editingProduct) {
                ...
                await axios.post(`http://localhost:8000/api/top-products/${editingProduct.id}`, submitData, {
                    headers: {
                ...
                await axios.post('http://localhost:8000/api/top-products', submitData, {
    ...
    const deleteProduct = async (id) => {
        if (window.confirm('Are you sure you want to delete this product?')) {
            try {
                await axios.delete(`http://localhost:8000/api/top-products/${id}`);
                fetchProducts();
            } catch (error) {
    ...
    const toggleStatus = async (product) => {
        try {
            await axios.patch(`http://localhost:8000/api/top-products/${product.id}/toggle-status`);
            fetchProducts();
        } catch (error) {
    ...
    const updateOrder = async (draggedId, targetId) => {
        ...
        try {
            await axios.patch('http://localhost:8000/api/top-products/update-order', {
                products: orderData
            });
    ...
    return (
        <div className="space-y-6">
            <div className="flex justify-between items-center">
                <h2 className="text-2xl font-bold text-gray-900">Top Products Management</h2>
                <button
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                            ...
                            <div>
                                <label className="block text-sm font-medium text-gray-700">
                                    {editingProduct ? 'Update Image' : 'Image *'}
                                </label>
                                <input
                                    type="file"
                                    accept="image/*"
                                ...
                                {imageFile && (
                                    <div className="mt-2">
                                        ...
                                        <img src={URL.createObjectURL(imageFile)} alt="Preview" className="h-20 object-cover rounded" />
                                    </div>
                                )}
                            </div>
                        </div>
                            ...
                            >
                                <div className="flex items-center space-x-4">
                                    <div className="flex flex-col space-y-1">
                                        <button
                                            onClick={() => index > 0 && updateOrder(product.id, products[index - 1].id)}
                                        ...
                                        <button
                                            onClick={() => index < products.length - 1 && updateOrder(product.id, products[index + 1].id)}
                                            className="text-gray-400 hover:text-gray-600 disabled:opacity-30"
                                    ...
                                    <div>
                                        ...
                                        <p className="text-sm font-bold" style={{
                                            color: product.theme_color === 'primary' ? themeColors.primary : themeColors.secondary
                                        }}>
                                            {product.price}
                                        </p>
                                    </div>
                ...
                <div className="flex flex-col gap-4">
                    {products.filter(p => p.is_active).length === 0 ? (
                        ...
                        products.filter(p => p.is_active).map((product) => (
                            <div key={product.id} className="relative bg-white rounded-2xl shadow-xl border border-gray-100 flex overflow-hidden group transition-all duration-300 hover:shadow-2xl hover:scale-[1.01] h-[200px]">
                                <div
                                    className="w-1/2 flex items-center justify-center p-4"
                                    style={{ backgroundColor: product.theme_color === 'primary' ? `${themeColors.primary}10` : `${themeColors.secondary}10` }}
                                >
                                    <img
                                        src={product.image}
                                        alt={product.title}
                                ...
                                <div className="w-1/2 flex flex-col justify-center p-6 space-y-2">
                                    ...
                                    <button
                                        ...
                                        style={{
                                            backgroundColor: product.theme_color === 'primary' ? themeColors.primary : themeColors.secondary,
                                        }}
                                    >
                                        {product.button_text}
                                    </button>
