...
const StatusUpdateModal = ({ isOpen, onClose, order, onStatusUpdate }) => {
    ...
    const statusOptions = [
        { value: 'pending', label: 'Pending', color: 'bg-amber-500', description: 'Order received, awaiting processing' },
        { value: 'contacted', label: 'Contacted', color: 'bg-blue-500', description: 'Customer has been contacted' },
        { value: 'processing', label: 'Processing', color: 'bg-indigo-500', description: 'Order is being processed' },
        { value: 'shipped', label: 'Shipped', color: 'bg-purple-500', description: 'Order has been shipped' },
        { value: 'completed', label: 'Completed', color: 'bg-green-500', description: 'Order delivered successfully' },
    ...
    const getCurrentStatusIndex = () => {
        return statusOptions.findIndex(option => option.value === order.status);
    };

    const getSelectedStatusIndex = () => {
        return statusOptions.findIndex(option => option.value === selectedStatus);
    };

    const isStatusAllowed = (statusValue) => {
        const currentIndex = getCurrentStatusIndex();
        const targetIndex = statusOptions.findIndex(option => option.value === statusValue);

        // Allow moving forward, backward, or staying same
        return true; // You can add business logic here if needed
    };
        ...
        <div className="fixed inset-0 z-50 overflow-y-auto">
            ...
            <div className="flex min-h-full items-center justify-center p-4">
                <div className="relative bg-white rounded-xl shadow-xl max-w-md w-full">
                    ...
                    <div className="p-6 border-b border-gray-200">
                        <div className="flex items-center justify-between">
                            <div>
                                ...
                                <div className={`inline-flex items-center gap-2 px-3 py-2 rounded-lg border mt-2 ${statusColor(order.status)}`}>
                            ...
                            <div className="text-right">
                                ...
                                <p className="text-sm text-gray-900 mt-2">{order.user?.name}</p>
                            </div>
                        </div>
                    </div>
                    ...
                    {/* Status Options */}
                    <div className="p-6">
                        <p className="text-sm font-medium text-gray-900 mb-4">Select New Status</p>
                            ...
                            {statusOptions.map((status) => (
                                <div
                                    ...
                                    className={`relative flex items-start p-4 border rounded-lg cursor-pointer transition-all ${
                                        ...
                                        !isStatusAllowed(status.value) ? 'opacity-50 cursor-not-allowed' : ''
                                    }`}
                                    onClick={() => isStatusAllowed(status.value) && setSelectedStatus(status.value)}
                                >
                                    <div className="flex items-center h-5">
                                        <input
                                            type="radio"
                                            checked={selectedStatus === status.value}
                                            onChange={() => isStatusAllowed(status.value) && setSelectedStatus(status.value)}
                                            className="h-4 w-4 text-blue-600 border-gray-300 focus:ring-blue-500"
                                            disabled={!isStatusAllowed(status.value)}
                                        />
                                    </div>
