...
const InvoicePDF = {
    generate: async (orderData) => {
        ...
        try {
            ...
            pdf.text(`Order #: ${order.order_code || order.id}`, pageWidth - margin, yPosition, { align: "right" });
            pdf.text(`Date: ${InvoicePDF.formatDate(order.created_at)}`, pageWidth - margin, yPosition + 5, { align: "right" });
            pdf.text(`Status: Confirmed`, pageWidth - margin, yPosition + 10, { align: "right" });
            ...
            pdf.setFont("helvetica", "normal");
            pdf.text(deliveryOption?.name || "Standard Delivery", margin + 20, yPosition);
            ...
            pdf.text("Qty", margin + colWidth * 2, yPosition + 5, { align: "center" });
            pdf.text("Unit Price", margin + colWidth * 2.5, yPosition + 5, { align: "right" });
            ...
            items.forEach((item, index) => {
                ...
                // Quantity
                pdf.text(item.quantity.toString(), margin + colWidth * 2, yPosition, { align: "center" });
                ...
                pdf.text(`Rs. ${unitPrice}`, margin + colWidth * 2.5, yPosition, { align: "right" });
                ...
                // Total
                pdf.text(`Rs. ${total}`, pageWidth - margin - 5, yPosition, { align: "right" });
                ...
                if (productHasDiscount) {
                    ...
                    const originalPrice = parseFloat(item.product.price).toFixed(2);
                    const savings = getProductSavings(item.product, item.quantity).toFixed(2);
                    pdf.text(`(Original: Rs. ${originalPrice} - Saved: Rs. ${savings})`, margin + 5, yPosition);
                    pdf.setFontSize(8);
                    pdf.setTextColor(0, 0, 0);
                    yPosition += 2;
                }
            ...
            if (totalSavings > 0) {
                ...
                pdf.text(`-Rs. ${totalSavings.toFixed(2)}`, pageWidth - margin - 5, yPosition, { align: "right" });
                yPosition += 5;
                pdf.setTextColor(0, 0, 0); // Reset to black
            }

            pdf.text("Subtotal:", summaryX, yPosition);
            pdf.text(`Rs. ${orderSummary.itemsTotal.toFixed(2)}`, pageWidth - margin - 5, yPosition, { align: "right" });

            pdf.text("Delivery Fee:", summaryX, yPosition + 5);
            pdf.text(`Rs. ${orderSummary.deliveryFee.toFixed(2)}`, pageWidth - margin - 5, yPosition + 5, { align: "right" });
            ...
            pdf.text(`Rs. ${orderSummary.total.toFixed(2)}`, pageWidth - margin - 5, yPosition, { align: "right" });
            ...
            if (totalSavings > 0) {
                pdf.text(`You saved Rs. ${totalSavings.toFixed(2)} on this order!`, pageWidth / 2, yPosition, { align: "center" });
                yPosition += 4;
            }

            pdf.text("Thank you for your business!", pageWidth / 2, yPosition, { align: "center" });
            pdf.text("This is a computer-generated invoice.", pageWidth / 2, yPosition + 4, { align: "center" });
    ...
    generateInvoiceHTML: (orderData) => {
        ...
        return `
            ...
            const productPrice = getProductPrice(item.product);
            const productHasDiscount = hasDiscount(item.product);
            const originalPrice = item.product?.price ? parseFloat(item.product.price).toFixed(2) : "0.00";
            const savings = getProductSavings(item.product, item.quantity).toFixed(2);
