...
function RecentOrders() {
    ...
    const openCancelModal = async (order) => {
        ...
        if (eligibility?.is_eligible) {
            ...
            const message = eligibility?.cancellation_eligibility?.is_within_time_limit === false
    ...
    const handleCancelOrder = async () => {
        if (!orderToCancel || !cancelReason.trim()) {
            showError('Please provide a reason for cancellation', 'Reason Required');
            return;
        }
    ...
    const canShowReviewButton = (product, orderStatus) => {
    ...
    const getTimeRemaining = (order) => {
        ...
        const timeDifference = Math.floor((currentTime - orderCreationTime) / (1000 * 60));
    ...
    const canCancelOrder = (order) => {
        ...
        return allowedStatuses.includes(order.status) && getTimeRemaining(order) > 0;
    };
                                                ...
                                                <div>
                                                    ...
                                                    {canCancel && (
                                                        <p className="text-xs text-orange-600 mt-1">
                                                            ⏰ Cancellation available for {timeRemaining} more minutes
                                                        </p>
                                                    )}
                                                </div>
                                            </div>
                                        ...
                                        <div className="flex items-center justify-between mt-4 pt-4 border-t border-gray-100">
                                            ...
                                            {order.status === 'cancelled' && order.cancellation_reason && (
                                                <div className="text-xs text-gray-500 max-w-xs">
                                                    ...
                                                    {order.cancelled_at && (
                                                        <p>Cancelled on: {formatDate(order.cancelled_at)}</p>
                                                    )}
                                                </div>
                                            )}
                                        </div>
