...
const OrderDetailsModal = ({ orderData, onClose, onReviewProduct, getStatusBadge, formatDate, formatCurrency }) => {
                    ...
                    <div className="grid grid-cols-2 gap-4 text-sm">
                        <div>
                            ...
                            {order.cancellation_reason && (
                                <p><span className="text-gray-600">Cancellation Reason:</span> {order.cancellation_reason}</p>
                            )}
                        </div>
                        <div>
                            <h3 className="font-semibold text-gray-900 mb-2">Payment & Delivery</h3>
                            <p><span className="text-gray-600">Total Amount:</span> {formatCurrency(parseFloat(order.total_amount))}</p>
                            <p><span className="text-gray-600">Delivery:</span> {order.delivery_option?.replace(/_/g, ' ').toUpperCase() || 'Standard'}</p>
                            <p><span className="text-gray-600">Delivery Fee:</span> {formatCurrency(parseFloat(order.delivery_fee || 0))}</p>
                        </div>
                    </div>
